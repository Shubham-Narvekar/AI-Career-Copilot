...
export const getQuestions = async (req: Request, res: Response) => {
    try {
        const questions = await AssessmentQuestion.find().populate('skill');
        res.json(questions);
    } catch (err) {
...
export const getQuestionById = async (req: Request, res: Response) => {
    try {
        const question = await AssessmentQuestion.findById(req.params.id).populate('skill');
...
export const createQuestion = async (req: Request, res: Response) => {
    try {
        ...
        const question = await AssessmentQuestion.create({ questionText, options, correctAnswer, skill });
...
export const updateQuestion = async (req: Request, res: Response) => {
    try {
        ...
        const question = await AssessmentQuestion.findByIdAndUpdate(
            req.params.id,
            { questionText, options, correctAnswer, skill },
            { new: true }
        );
        if (!question) return res.status(404).json({ message: 'Question not found' });
        res.json(question);
...
export const submitResult = async (req: Request, res: Response) => {
    try {
        ...
        const result = await AssessmentResult.create({ user, answers, score });
...
export const getResultsByUser = async (req: Request, res: Response) => {
    try {
        const results = await AssessmentResult.find({ user: req.params.userId })
            .populate('user')
...
const generateFallbackQuestions = (skill: string, category: string, count: number, difficulty: string) => {
    ...
    const categoryQuestions = skillSpecificQuestions[category as keyof typeof skillSpecificQuestions] || skillSpecificQuestions['Programming'];
...
export const generateRecommendations = async (req: Request, res: Response) => {
    try {
        ...
        const recommendations = await geminiService.generateSkillRecommendations(assessmentResults);
...
export const generateLearningPath = async (req: Request, res: Response) => {
    try {
        ...
        const learningPath = await geminiService.generateLearningPath(skill, currentLevel);
...
const generateFallbackRecommendations = (assessmentResults: { skill: string; score: number }[]) => {
...
const generateFallbackLearningPath = (skill: string, currentLevel: string) => {
    const levelSteps = {
        beginner: [
            {
    ...
    const steps = levelSteps[currentLevel as keyof typeof levelSteps] || levelSteps.intermediate;
    ...
    try {
        ...
        if (hasApiKey) {
            try {
                ...
                const testQuestions = await geminiService.generateAssessmentQuestions(
                    'JavaScript',
                    'Programming',
                    1,
                    'beginner'
                ...
                testResult = {
                    ...
                    sampleQuestion: testQuestions[0]?.questionText?.substring(0, 50) + '...'
                };
            } catch (error) {
                testResult = {
                    hasApiKey,
